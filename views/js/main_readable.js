...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    //Since the pizza sizes are all the same, every element will have a newwidth the same as the first element.
    //Therefore, I am just retrieving the values once and then resetting all the elements to the same newwidth.
...
function updatePositions() {
  ...
  //The movement of the pizzas in the background will have 1 of 5 values.
  //Since those calculations are somewhat intensive, we will calculate those values once which
  //can then be used when iterating through all the moving elements.
  var phase = [];
  for (var i = 0; i <= 4; i++){
...
//global variable to be used to hold all mover classes.
var items=null;
// Generates the sliding pizzas when the page loads.
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //Reduced the number of pizzas drawn from 200 to 50 since that is plenty to cover the largest screen.
  for (var i = 0; i < 50; i++) {
  ...
  //Initiated a global variable for all the mover classes.  No need to do it each time the UpdatePositions function is called.
